library(ggplot2)
data <- read.csv("dsrC-dsrMK.csv", header = T, check.names = F)
data$phylum <- factor(data$phylum,levels = c("Proteobacteria","Latescibacterota"))
mycol <- c("#d4a6c8","#d7b5a6")
...
ggsave("dsrC-dsrMK.pdf",p,width = 8,height = 8)