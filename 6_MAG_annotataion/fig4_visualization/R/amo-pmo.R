library(ggplot2)
data <- read.csv("amo-pmo.csv", header = T, check.names = F)
data$phylum <- factor(data$phylum,levels = c("Proteobacteria","Thermoproteota"))
mycol <- c("#d4a6c8","#00a98f")
...
ggsave("amo-pmo.pdf",p,width = 8,height = 8)