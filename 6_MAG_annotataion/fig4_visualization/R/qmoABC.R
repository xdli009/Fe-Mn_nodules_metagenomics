library(ggplot2)
data <- read.csv("qmoABC.csv", header = T, check.names = F)
data$phylum <- factor(data$phylum,levels = c("Desulfobacterota_B","Proteobacteria","Acidobacteriota"))
mycol <- c("#fabfd2","#d4a6c8","#4e79a7")
...
ggsave("qmoABC.pdf",p,width = 8,height = 8)